(function(root){
	...
	root.ODI.CoastLine = function(id,options){
		...
		this.init = function(fn){
			...
			else this.svg.classList.remove('shape');
			this.svg.innerHTML += '<style>path.coast { fill: #efefef; }svg:not(.shape) rect { transform: scale(1)!important; } svg:not(.shape) .label { transform: scale(1,-1)!important; } svg:not(.shape) rect, svg:not(.shape) .label { transition: transform 1s ease 0s, height 0.5s ease 1s, fill 1.5s ease 0s;} svg.shape rect, svg.shape .label { cursor: pointer; height: 6px; rx: 3px; transition: transform 1s ease 0.5s, height 0.5s ease 0s, fill 1.5s ease 0s!important; } svg.shape.resizing rect, svg.shape.resizing .label { transition: unset!important; } svg text { font-size:16px;font-family:Arial;font-weight:bold; transform: translate(8px,0); dominant-baseline: middle; text-anchor: start; } svg.shape text.left { transform: translate(-8px,0); text-anchor: end; } svg.shape text.top { transform: translate(0,-14px); text-anchor: middle; } svg.shape text.bottom { transform: translate(0,14px); text-anchor: middle; }</style>';
			this.setSize(this.el.clientWidth,this.el.clientHeight);
